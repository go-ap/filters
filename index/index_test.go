	...
	}{
		{
			...
			want: &Index{
				...
				Indexes: map[Type]Indexable{
					ByType:              NewTokenIndex(ExtractType),
					ByName:              NewTokenIndex(ExtractName),
					ByPreferredUsername: NewTokenIndex(ExtractPreferredUsername),
					BySummary:           NewTokenIndex(ExtractSummary),
					ByContent:           NewTokenIndex(ExtractContent),
					ByActor:             NewTokenIndex(ExtractActor),
					ByObject:            NewTokenIndex(ExtractObject),
					ByRecipients:        NewTokenIndex(ExtractRecipients),
					ByAttributedTo:      NewTokenIndex(ExtractAttributedTo),
				},
			},
		},
	}
...
var emptyFullIndex = []byte{
	...
	0x74, 0x36, 0x34, 0x5d, 0x61, 0x63, 0x74, 0x69, 0x76, 0x69, 0x74, 0x79, 0x70, 0x75, 0x62, 0x2e, 0x49, 0x52, 0x49,
...
func TestIndex_UnmarshalBinary(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:    "empty",
			arg:     emptyFullIndex,
			want:    Index{Ref: make(map[uint64]vocab.IRI), Indexes: make(map[Type]Indexable)},
			wantErr: false,
		},
	}
	for _, tt := range tests {
